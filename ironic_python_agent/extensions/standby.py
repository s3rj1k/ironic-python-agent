...
def _execute_configdrive_hook(configdrive, device, image_info, stage):
    ...
    """
    LOG.debug('_execute_configdrive_hook called with stage: %s', stage)

    if not configdrive:
        LOG.debug('No configdrive provided, skipping %s hook', stage)
        return
    ...
    try:
        ...
        LOG.debug('Configdrive file created at: %s', confdrive_file)
        partition_utils.execute_configdrive_deploy_hook(
            confdrive_file, device, stage)
    except errors.DeploymentError as e:
        # Only re-raise deployment errors from configdrive processing if
        # they're related to script execution, not configdrive format issues
        if 'not base64 encoded' in str(e) or 'malformed' in str(e):
            LOG.error('Configdrive format issue, skipping %s hook: %s',
                      stage, e)
            return
        raise
        ...
        LOG.error('%s-deployment hook execution failed: %s',
                  stage.capitalize(), e)
        error_msg = f'{stage.capitalize()}-deployment hook execution failed.'
        if e.stderr:
            error_msg += f' [{e.stderr.strip()}'
            if e.exit_code is not None:
                error_msg += f' (exit {e.exit_code})]'
            else:
                error_msg += ']'
        elif e.exit_code is not None:
            error_msg += f' [exit {e.exit_code}]'
        if e.stdout:
            error_msg += f' Script output: {e.stdout}'
        raise errors.DeploymentError(error_msg) from e
    except Exception as e:
        LOG.error('Failed to process configdrive for %s-deployment '
                  'hook execution: %s', stage, e)
        raise errors.DeploymentError(
            f'Failed to process configdrive for {stage}-deployment hook'
        ) from e
    finally:
        if confdrive_file:
            LOG.debug('Cleaning up configdrive file: %s', confdrive_file)
