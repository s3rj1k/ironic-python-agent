...
def get_oci_image(configdrive_data):
    ...
    oci_image = meta_data.get('oci_image', DEFAULT_OCI_IMAGE)
...
def clean_device(device):
    ...
    try:
        result = run_command(['pvs', device], check=False)
        if result.returncode == 0:
            vg_result = run_command(
                ['pvs', '--noheadings', '-o', 'vg_name', device],
                check=False
            )
            ...
            if vg_name:
                ...
                for lv_path in lv_result.stdout.strip().split('\n'):
                    ...
                    if lv_path:
                        run_command(['lvremove', '-f', lv_path], check=False)

                run_command(['vgremove', '-f', vg_name], check=False)

            run_command(['pvremove', '-ff', '-y', device], check=False)
    except Exception:
        pass
...
def install_packages(chroot_dir, grub_packages):
    ...
    packages = [
        'cloud-init', 'efibootmgr', 'grub-common', 'initramfs-tools',
        'lvm2', 'mdadm', 'rsync', 'systemd-sysv'
    ] + grub_packages
