...
def execute_configdrive_deploy_hook(confdrive_file, device, stage):
    ...
    then executes the script from /opt/bin/.
    ...
    :param stage: Stage of deployment ('pre' or 'post') passed as env variable
    :raises: ProcessExecutionError if script execution fails
    :raises: DeploymentError if script is missing from /opt/bin/
    """
    conf_drive_temp = tempfile.mkdtemp()
    try:
        utils.execute('mount', '-o', 'loop,ro', '-t', 'auto',
    ...
    try:
        ...
        if not os.path.exists(metadata_path):
            LOG.debug('No metadata file found at %s, skipping', metadata_path)
            return
        ...
        if not isinstance(script_name, str):
            LOG.warning('Invalid deploy_hook value, expected string')
            return

        script_path = os.path.join('/opt/bin', script_name)
        ...
        try:
            ...
            LOG.warning('Failed to unmount configdrive temp directory: %s', e)
        utils.unlink_without_raise(conf_drive_temp)
